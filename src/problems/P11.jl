ans() = begin
    NB_ADJACENT = 4
    right_prod(matrix, row, col) = reduce(*, [matrix[row][col+i] for i in 0:3])
    down_prod(matrix, row, col) = reduce(*, [matrix[row+i][col] for i in 0:3])
    ...
    f = open("data/p11.txt")
    lines = readlines(f)
    matrix = [[parse(Int, col) for col in split(line, ' ')] for line in lines]
    products = []
    for row in 1:20
        for col in 1:20
            if right_valid(col)
